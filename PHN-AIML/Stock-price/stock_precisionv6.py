...
/* Only target <p> — NEVER <span> inside summary.
   Streamlit renders the collapse arrow as a glyph in a <span> via
   Material Icons font. Forcing Inter on that span replaces the glyph
   with literal text like "arrow_drop_down". */
...
PL = dict(
    ...
    # Increase t (top margin) to 85 to clear the floating toolbar
...
# Change y to 1.12 to move the legend away from the hover icons
fig_p.update_layout(**PL, height=550,
...
with st.expander("RSI & MACD Technical Indicators", expanded=False):
    ...
    # Override margin: t=60 so subplot titles (RSI/MACD) sit below the legend
    # without overlapping. vertical_spacing=0.10 keeps the two panels separated.
    # Update legend y to 1.12 and set top margin to 85
    fig_i.update_layout(**{**PL, 'height': 450, 'margin': dict(l=0, r=0, t=85, b=0)},
                        legend=dict(orientation='h', yanchor='bottom', y=1.12, xanchor='right', x=1,
                                    bgcolor='rgba(8,12,20,0.85)', bordercolor='#1e2d40', borderwidth=1))
    # Style subplot title annotations
    for ann in fig_i.layout.annotations:
    ...
    st.plotly_chart(fig_i, use_container_width=True, config={'displaylogo': False})
