    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                cx_middle = int(middle_tip.x * w)
                cy_middle = int(middle_tip.y * h)

                # volume control
                gesture["volume"] = np.interp(cy_middle, [0, h], [1.5, 0.2])

                # distance (tone control)
                dist = np.sqrt((cx_index - cx_thumb) ** 2 + (cy_index - cy_thumb) ** 2)
                gesture["tone"] = np.interp(dist, [20, 300], [0.5, 2.0])

                gesture["pitch"] = np.interp(cy_index, [0, h], [12.0, -12.0])
                gesture["speed"] = np.interp(cx_thumb, [0, w], [0.5, 2.0])

                cv2.circle(frame, (cx_index, cy_index), 12, (0, 255, 0), -1)  # pitch
                cv2.circle(frame, (cx_thumb, cy_thumb), 12, (255, 0, 0), -1)  # speed
                cv2.circle(frame, (cx_middle, cy_middle), 12, (0, 0, 255), -1)  # volume

                # line for tone control
                cv2.line(frame, (cx_index, cy_index), (cx_thumb, cy_thumb), (255, 255, 0), 2)
        ...
        print(f"vol={gesture['volume']:.2f}, tone={gesture['tone']:.2f}")
